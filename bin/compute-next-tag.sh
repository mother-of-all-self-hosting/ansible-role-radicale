...
#
# Tags look like `v<Radicale image version>-<release>`:
#
# - if defaults/main.yml points at a version that has never been released, the
#   release counter restarts at 0 (`v3.7.6.0-0`)
# - otherwise the counter is incremented (`v3.7.6.0-1`), but only if something
...
# `radicale_version` is the literal that Renovate bumps, and the only place the
# version is spelled out. Everything else (the image tag, the self-build branch
# name) is derived from it in Jinja, which cannot be read out of the file.
version="$(sed -nE 's|^radicale_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Radicale version from $defaults_path"
	exit 1
fi

# The tags carry a leading `v` that the version value itself does not, and
# doubling it must not happen should the value ever gain one.
